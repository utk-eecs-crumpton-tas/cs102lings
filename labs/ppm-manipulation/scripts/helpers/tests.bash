test_exit_status() {
    ...
    if (($exit_status == 0 && $tmp_exit_status != 0)); then
        echo
        echo -e "${ERROR_STYLE}Error:${RESET} received unexpected non-zero exit status: ${CODE_STYLE}$tmp_exit_status${RESET}" >&2
        echo
        if [[ -f "$tmp_stderr_path" && -s "$tmp_stderr_path" ]]; then
            cat "$tmp_stderr_path" >&2
            echo
        fi
        return 1
    fi
