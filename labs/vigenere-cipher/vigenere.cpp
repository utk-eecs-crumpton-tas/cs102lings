 ...
 * SPANS MULTIPLE LINES. REFER TO THE TAs IF YOU HAVE
 * ANY QUESTIONS.
 * ***************************************************** */
...
void VigenereCipher::print() const {
    ...
    cerr << "Key: " << vigenereKey << endl;
    cerr << "Table:" << endl;
    for (size_t i = 0; i < ALPHABET_SIZE; i++) {
        for (size_t j = 0; j < ALPHABET_SIZE; j++) {
            cerr << " " << vigenereTable[i][j] << " ";
        }
        cerr << endl;
    }
}
...
string VigenereCipher::makeKeystream(const string message) const {
    ...
    for (size_t i = 0; i < message.size(); i++) {
        if (islower(message[i])) {
            // TODO: add a character from the key (vigenereKey) to the keystream
        } else {
...
void VigenereCipher::encrypt(const string inputFile,
    ...
    // - Use the same `for if islower else` loop structure as in `makeKeystream`
...
/* *****************************************************
 * Function: Vigenere Cipher Constructor
 * Description: WRITE YOUR OWN DESCRIPTION HERE. IT
 * SPANS MULTIPLE LINES. REFER TO THE TAs IF YOU HAVE
 * ANY QUESTIONS.
 * ***************************************************** */
VigenereCipher::VigenereCipher(const string key) {
    // - Store the key into `vigenereKey`
    // - Initialize `vigenereTable`
    // - Use the same for loops structure as `VigenereCipher::print`
    //   to initialize `vigenereTable`

    // NOTE: Since `vigenereKey` and `vigenereTable` are private, they accessible in any
...
int main(int argc, char **argv) {
    ...
    // TODO: Call encrypt or decrypt based on the operation
